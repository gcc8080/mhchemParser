...
- Add a type-safe mode API with deterministic invalid-mode errors while retaining the current string-based entry point as a deprecated compatibility wrapper.
- Add an opt-in, depth-bounded helper for recursively expanding embedded `\\ce{...}` and `\\pu{...}` commands without changing the exact upstream behavior of the primary conversion API.
