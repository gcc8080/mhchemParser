...
- [ ] 3.1 Add `MhchemMode` and `MhchemParser.convert(String input, {required MhchemMode mode})` as the typed, single-pass entry point that centralizes all supported-mode dispatch.
- [ ] 3.2 Convert `MhchemParser.toTex(String input, String type)` into a deprecated delegating wrapper retained throughout `0.x`, and test valid-mode parity plus deterministic `ArgumentError` details for invalid modes.
- [ ] 3.3 Add `MhchemParser.expandAllTex(String input, {int maxPasses = 16})` with caller validation, convergence detection, and public `MhchemExpansionException` pass-limit/non-progress reasons.
...
- [ ] 6.3 Run the complete minimum/stable verification suite, record all 117 JS and Dart cases passing, run `openspec validate align-mhchem-parser-4-2-2-conformance --strict`, and prepare release notes without creating a tag, GitHub release, or pub.dev publication.
